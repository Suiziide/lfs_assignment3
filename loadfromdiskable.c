...
void SaveAux(struct LinkedListNode *node, FILE *fp, char *savePath){
    if (node == NULL) {return;}
    size_t len;
    if (node != root){
    ...
    fprintf(fp, "%ld|", node->entry->mod_time);
    fprintf(fp, "%ld|", node->entry->access_time);
    if(node->entry->contents && strlen(node->entry->contents) != 0){
...
int loadFromDisk() { 
    FILE *fp;
    if ((fp = fopen("disk.img", "r")) == NULL) { return 1; }

    fseek(fp, 0, SEEK_END);
    int filesize = ftell(fp);
    rewind(fp);
    char fBuf[filesize+1];  // +1 might be redundant FIXME
...
//                  printf("case 5\n");
                    strncpy(temp, &fBuf[oldPipe+1], (size_t) (i-oldPipe-1));
                    temp[i-oldPipe-1] = '\0';
                    access_time = (u_int64_t) atoi(temp);
//                  printf("acc_time: %s\n", temp);
                    break;
                case 6: // mod_time
//                  printf("case 6\n");
...
//              if (contents) { printf("CONTENTS: %s\n", contents); }
                ...
                current->entry->access_time = access_time;
                current->entry->mod_time = mod_time;
