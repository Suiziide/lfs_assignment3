...
int lfs_mkdir(const char *path, mode_t mode) {
    ...
    if (!seekOp && token == NULL) {
        ...
        new_node->entry->id = 42; //FIXME - FUNCTION
...
int lsf_mknod(const char *path, mode_t mode, dev_t device) {
    ...
    if (!seekOp && token == NULL) {
        ...
        new_node->prev = parent->entry->files->tail;
        if (new_node->prev) {
            new_node->prev->next = new_node; // actually parent->entry->subdirs->tail->next
        }
        parent->entry->files->tail = new_node;
        if (!parent->entry->files->head) {
            parent->entry->files->head = new_node;
        }
        ++parent->entry->files->num_entries;
    } else {
        free(current_path);
        return -EEXIST;
    }
